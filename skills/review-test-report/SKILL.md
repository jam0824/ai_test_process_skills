...
3. Review the report using the review perspectives below.
4. List findings with priority, grounded in concrete report files and source artifacts.
5. Fix all `P0`, `P1`, and `P2` findings that are actually fixable from available information. Fix `P3` findings only when the correction is low-risk and clearly supported.
6. Re-review the edited or regenerated report. Repeat the review and fix loop until no `P0`, `P1`, or `P2` findings remain, except when a finding cannot be fixed because required information is unavailable. In that case, record the unresolved item clearly.
7. Save the final review result as Markdown. If the user does not specify a path, save it as `テスト成果物/テストレポートレビュー結果.md`.
...
- **Aggregation semantics**: Confirm formulas are documented and correct. Pass rate should exclude `N/A` when defined as `Pass / (Pass + Fail)`. Distinguish not-run, N/A, question-wait, unimplemented, skipped, excluded, and human-run-not-yet-executed states when the source artifacts distinguish them.
...
- **Security and privacy**: Confirm the report does not expose credentials, API keys, tokens, personal information, secret URLs, environment dumps, or excessive local machine details. Redact or mark as internal-only when needed.
...
- Treat secret/token exposure as `P0`; treat unnecessary local absolute path exposure as `P2` unless it leaks sensitive information.
