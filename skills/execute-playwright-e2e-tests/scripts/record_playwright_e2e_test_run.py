...
def main(argv: list[str]) -> int:
    ...
    report_dir = report_root / f"{timestamp}_e2e自動テスト"
    report_dir.mkdir(parents=True, exist_ok=True)
    issue_file = Path(args.issue_file) if args.issue_file else report_dir / "e2e_発見issue一覧.md"
    executed_md = report_dir / f"{timestamp}_テストケース_E2E自動_実行済み.md"
    executed_csv = report_dir / f"{timestamp}_テストケース_E2E自動_実行済み.csv"
    raw_log = report_dir / f"{timestamp}_E2E自動テスト実行ログ.txt"
    json_result = report_dir / f"{timestamp}_Playwright_E2E実行結果.json"
